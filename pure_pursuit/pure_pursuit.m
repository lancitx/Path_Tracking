...
% 绘制参考轨迹
figure
plot(RefPos(:,1), RefPos(:,2), 'color', '#80B3FF', 'LineWidth', 2);
xlabel('纵向坐标 / m');
ylabel('横向坐标 / m');
grid on;
grid minor
...
diff_x(end+1) = diff_x(end); % 补齐向量
diff_y = diff(RefPos(:,2)) ;
diff_y(end+1) = diff_y(end);
refHeading = atan2(diff_y , diff_x); % 参考航向角

%% 车辆初始状态
pos = RefPos(1, :)+1; % 初始点偏移[1, 1]
v = 0; % 速度
heading = refHeading(1) + 0.02; % 初始航向偏移0.02
...
while idx < sizeOfRefPos-1  % 由于diff_x，diff_y计算方式的特殊性，不考虑最后一个参考点
    ...
    % 前轮转角 PI控制
    Err_integ = Err_integ + latError * dt; % 积分误差
...
for i = 1:size(pos_actual,1)-1
    scatter(pos_actual(i,1), pos_actual(i,2), 200, [0 0.7 0.7], '.');    % 实际位置(x,y)
    % 绘制预瞄点
...
function [lookaheadPoint,idx_target]=findLookaheadPoint(pos, v, RefPos, Kv, Ld0)
    ...
    for i = 1:sizeOfRefPos-1
        dist(i,1) = norm(RefPos(i,:) - pos); % 计算路径点到欧几里得距离   
    end
    [~,idx] = min(dist); 
    ...
    while L_steps < Ld && idx < sizeOfRefPos
        ...
        % 新定义，L_steps为路径上的点到当前位置的距离norm(RefPos(idx + 1,:) - pos)
        % L_steps = norm(RefPos(idx + 1,:) - pos);
        idx = idx+1;
    end
    idx_target = idx; % 返回索引
    lookaheadPoint = RefPos(idx,:); % 返回离预瞄点最近的路径点
end
...
function [delta,latError] = pure_pursuit_control(lookaheadPoint, idx, pos, heading, v, RefPos, refHeading, Kv, Ld0, L)
    ...
    if idx < sizeOfRefPos
        ...
        Point_temp = RefPos(end,1:2); % 提取最后一个点
    end
...
%     latError = y_error*cos(heading_r) - x_error*sin(heading_r); % 根据百度Apolo，计算横向误差
